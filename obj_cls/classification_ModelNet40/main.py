...
from utils import Logger, mkdir_p, progress_bar, save_model, save_args, cal_loss, count_parameters
...
def main():
    ...
    run_name = args.model + message + '-' + str(args.seed)
    wandb.init(
        name=run_name, project="point-mixer-cls", config=args, entity="chrockey" # you need to modify entity
    )
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        if test_out["acc_avg"] > best_test_acc_avg:
            best_test_acc_avg = test_out["acc_avg"]
            is_best_avg = True
        else:
            is_best_avg = False
        ...
        log_dict = {
            ...
            "train/OA": train_out["acc"],
            "train/mAcc": train_out["acc_avg"],
            "train/best_OA": best_train_acc,
            "train/best_mAcc": best_train_acc_avg,
            "val/loss": test_out["loss"],
            "val/OA": test_out["acc"],
            "val/mAcc": test_out["acc_avg"],
            "val/best_OA": best_test_acc,
            "val/best_mAcc": best_test_acc_avg,
            "epoch": epoch
        }
        wandb.log(log_dict)

        save_model(
            net, epoch, path=args.checkpoint, acc=test_out["acc"], is_best=is_best, is_best_avg=is_best_avg,
